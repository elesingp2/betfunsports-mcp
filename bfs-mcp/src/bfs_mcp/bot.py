...
TOOLS = [
    ...
    _td("bfs_login", "Authenticate. Returns balances or error (e.g. 'Player already logged in').",
    ...
    _td("bfs_active_bets", "Get active (unresolved) bets — portfolio monitoring."),
...
2. bfs_login → если надо (если "Player already logged in" — объясни что нужно выйти из другого браузера)
...
## Мониторинг
- bfs_active_bets → текущие открытые позиции (ожидают результат)
- bfs_bet_history → CSV вся история (для анализа точности)
...
async def _exec(name: str, args: dict) -> tuple[str, bytes | None]:
    ...
    match name:
        ...
        case "bfs_active_bets":
            return j(await bm.active_bets())[:4000], None
